	...
	<script type="text/javascript">
		...
		$(function() {
			$('#dg').datagrid({
				...
				url : '${dynamicURL}/deploy/searchUnixInfo.action?id=${weblogicId}',
				columns:[[
				          {field:'ip',title:'服务器IP', width:60},
				          {field:'parentId',title:'父节点id', width:60,hidden:true},
				...
				rowStyler : function ( index, row ) {   
			        if ( row.parentId == 0 ) {
			        	if (row.unixUsername == "" || row.unixPassword == "" || row.ftpIp == "" || 
			        			row.ftpUsername == "" || row.ftpPassword == "") {
		...
		function saverow(tt){
			...
			var ipaddress=$("#dg").datagrid('getRows')[tt].ip;
			var parentIdNum=$("#dg").datagrid('getRows')[tt].parentId;
			 
			$('#fm').form('submit',{
                url: '${dynamicURL}/deploy/updateUnixInfo.action?ip='+ipaddress+'&uu='+name+'&up='+password+'&fi='+ftpIp+'&fu='+ftpUsername+'&fp='+ftpPassword+'&pi='+parentIdNum,
                onSubmit: function(){
